...
import saikaone.KeyListener;
import saikaone.Prefabs;
...
public class PlayerController extends Component {
    ...
    private enum PlayerState {
        ...
        public void update(float dt) {
            ...
            if(isDead) {
                if(this.gameObject.transform.position.y < deadMaxHeight && deadGoingUp) {
                    ...
                    this.acceleration.y = Window.getPhysics().getGravity().y * 3.7f;
                    this.velocity.y = this.acceleration.y * dt;
                    this.velocity.y  = Math.max(Math.min(this.velocity.y, this.terminalVelocity.y), -this.terminalVelocity.y);
                    this.rb.setVelocity(this.velocity);
                    this.rb.setAngularVelocity(0);
                } else if(!deadGoingUp && gameObject.transform.position.y <= deadMinHeight) {
                    Window.changeScene(new LevelSceneInitializer()); //sets back to original scene when dead
                }
                return;
            }
            ...
            if (KeyListener.isKeyPressed(GLFW_KEY_RIGHT) || KeyListener.isKeyPressed(GLFW_KEY_D)) {
                this.gameObject.transform.scale.x = playerWidth; //transform direction facing
                ...
                if (this.velocity.x > 0) {
                    this.velocity.x = Math.max(0, this.velocity.x - slowDownForce); //linear slowdown
            ...
            if(KeyListener.isKeyPressed(GLFW_KEY_SPACE) &&
               ...
               this.velocity.y = ((enemyBounce / 2.2f) * jumpBoost);
                ...
                if(this.jumpTime > 0) {
                    this.velocity.y *= 0.35f; //when releasing space, slow vertical velocity
                    this.jumpTime = 0;
                }
                groundDebounce -= dt;
                this.acceleration.y = Window.getPhysics().getGravity().y * 0.7f; //apply gravity only when falling
            } else {
                this.velocity.y = 0;
            ...
            if (KeyListener.keyBeginPress(GLFW_KEY_E) && playerState == PlayerState.Fire &&
                ...
                Vector2f position = new Vector2f(this.gameObject.transform.position)
                        ...
                        ? new Vector2f (0.26f, 0) //right spawning
                        : new Vector2f(-0.26f, 0)); //left spawning
        ...
        public void checkOnGround() {
            ...
            //returns weather player is grounded:
    ...
    public void beginCollision(GameObject collidingObject, Contact contact, Vector2f hitNormal) {
        ...
        if(collidingObject.getComponent(Ground.class) != null) {
            if(Math.abs(hitNormal.x) > 0.8f) { //hit block horizontal
                this.velocity.x = 0;
            } else if (hitNormal.y > 0.8f) { //hit block vertical
                this.velocity.y = 0;
                this.acceleration.y = 0;
                this.jumpTime = 0;
            }

...
public class TurtleAI extends Component{
    ...
    public void update(float dt) {
        ...
        if(this.gameObject.transform.position.x <
                ...
                //|| this.gameObject.transform.position.y < 0.0f) { //if fallen Offscreen
            this.gameObject.destroy();
        }
    }
    //TODO: MAKE A STATIC PHYSICS CONTROLLER FOR ALL CHARACTERS
    ...
    public void beginCollision(GameObject object, Contact contact, Vector2f hitNormal) {
        ...
        if(playerController != null) {
            if(!isDead && !playerController.isDead() && !playerController.isHurtInvincible() &&
                ...
                walkSpeed *= 3.0f;
                    ...
                    !playerController.isHurtInvincible() && (isMoving||!isDead) &&
                    hitNormal.y < 0.58f) {
        ...
        } else if(Math.abs(hitNormal.y) < 0.1f && !object.isDead()) {
            goingRight = hitNormal.x < 0;
            if(!isMoving && isDead) {
                AssetPool.getSound("assets/sounds/bump.ogg").play();
            }
        ...
        if(object.getComponent(Fireball.class) != null) {
            stomp();
            object.getComponent(Fireball.class).disappear();
        }
    }

    @Override
    public void preSolve(GameObject object, Contact contact, Vector2f hitNormal) {
        GoombaAI goomba = object.getComponent(GoombaAI.class);
        ...
        if (playerController != null) {
            if (!isDead && !playerController.isDead() &&
                ...
                && object.getComponent(MushroomAI.class) == null) {
            goingRight = hitNormal.x < 0;
            if (isMoving && isDead) {
                AssetPool.getSound("assets/sounds/bump.ogg").play();
            }

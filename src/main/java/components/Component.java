...
public abstract class Component {
    ...
    public void imgui() {
        try {
            Field[] fields = this.getClass().getDeclaredFields(); //gets fields from subclass
            for(Field field : fields) {
                boolean isTransient = Modifier.isTransient(field.getModifiers());
                if(isTransient) {
                    continue; //skip over transient keyword
                }
                boolean isPrivate = Modifier.isPrivate(field.getModifiers());
                if(isPrivate) {
                    field.setAccessible(true); //set private temporarily to public
                }
                ...
                if(type == int.class) {
                    ...
                    if(ImGui.dragFloat4(name + ": ", imVec)) {
                        ...
                        //REPLACE WITH JImGui.colorPicker4(name,val); !!!
                    }
                } else if(type.isEnum()) {

...
public class MouseControls extends Component{
    ...
    private float debouceTime = 0.2f;
    private float debounce = debouceTime;
    ...
    public void pickupObject(GameObject go) {
        ...
        this.holdingObject.getComponent(SpriteRender.class).setColor(
    ...
    public void editorUpdate(float dt) {
        ...
        if(holdingObject != null) {
            ...
            holdingObject.transform.position.x =
                    ((int)Math.floor(holdingObject.transform.position.x / Settings.GRID_WIDTH) * Settings.GRID_WIDTH) + Settings.GRID_WIDTH /2.0f;
            holdingObject.transform.position.y =
                    ((int)Math.floor(holdingObject.transform.position.y / Settings.GRID_HEIGHT) * Settings.GRID_HEIGHT) + Settings.GRID_HEIGHT/2.0f;

            if(MouseListener.mouseButtonDown(GLFW_MOUSE_BUTTON_LEFT)) {
                float halfWidth = Settings.GRID_WIDTH /2.0f;
                float halfHeight = Settings.GRID_HEIGHT /2.0f;
                if(MouseListener.isDragging() && !blockInSquare(
                        holdingObject.transform.position.x - halfWidth,

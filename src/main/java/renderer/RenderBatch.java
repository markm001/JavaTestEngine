...
public class RenderBatch implements Comparable<RenderBatch> {
    ...
    private final int VERTEX_SIZE = 10; //9floats inside 1 vertex
    ...
    public RenderBatch(int maxBatchSize, int zIndex, Renderer renderer) {
        ...
        //4 vertices quads - Size from Float in Vertex
    ...
    public void start() {
        ...
        glBufferData(GL_ARRAY_BUFFER,vertices.length * Float.BYTES, GL_DYNAMIC_DRAW);//change, so dynamic

        //Create & upload indices buffer
        int eboID = glGenBuffers();
        int[] indices = generateIndices();
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, eboID);
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, indices,GL_STATIC_DRAW); //never change, so static!
    ...
    public void addSprite(SpriteRender spr) {
        ...
        int index = this.numSprites; //put @ end of array
        this.sprites[index] = spr;
        this.numSprites++; //add next sprite in next slot

        //check if sprite has texture -> add texture to list
        ...
        //Add properties(float) to local vertices array
    ...
    public void render() {
        ...
        for(int i=0; i<numSprites; i++) { //loop through all sprites
        ...
        if(rebufferData) {
            glBindBuffer(GL_ARRAY_BUFFER, vboID);
            glBufferSubData(GL_ARRAY_BUFFER, 0, vertices); //buffer vertices into buffer starting at0
        }
        ...
        for(int i=0; i<textures.size();i++) {
            glActiveTexture(GL_TEXTURE0 + i + 1); //appropriate Slot
            textures.get(i).bind();
        }
    ...
    private void loadVertexProperties(int index) {
        ...
        //Find offset in array (4 vertices / sprite)
        ...
        if(sprite.getTexture() != null) {
            for (int i = 0; i < textures.size(); i++) {
                if(textures.get(i).equals(sprite.getTexture())) {
                    texId = i + 1; //0 reserved for flat color +1.. for textures
                    break;
                }
            }
        }
        ...
        if(isRotated) {
            ...
            transformMatrix.rotate((float)Math.toRadians(sprite.gameObject.transform.rotation),
                    0,0,1); //rotate along zAxis - 2D rotation
    ...
    public boolean hasTextureRoom() {
        return this.textures.size() < 8; //if less than 8 textures in render
    }
    ...
    public int compareTo(RenderBatch o) {
        return Integer.compare(this.zIndex, o.zIndex()); //compare batches based on zIndex
    }
}
